...
func (s *Server) handlerClosure(ctx context.Context, request *hcjson.Request) lazyHandler {
	...
	var rpcClient *hcrpcclient.Client
	if chainClient != nil {
		// The "help" RPC must use an HTTP POST client when calling down to hcd
...
// but this is only implemented here to work around an oddity with hcjson,
...
func (s *Server) invalidAuth(req *hcjson.Request) bool {
	cmd, err := hcjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*hcjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := hcjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) GetWallet() *loader.Loader {
	return s.walletLoader
}
