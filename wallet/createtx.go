...
func feeForSize(incr hcutil.Amount, sz int) hcutil.Amount {
	return hcutil.Amount(1+sz/1000) * incr
}
...
func (s *secretSource) GetKey(addr hcutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr hcutil.Address) ([]byte, error) {
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32, minconf int32,
	randomizeChangeIdx bool, changeAddr string, payLoad []byte, fromAddress string) (*txauthor.AuthoredTx, error) {
	...
	return w.txToOutputsInternal(outputs, account, minconf, chainClient,
		randomizeChangeIdx, w.RelayFee(), changeAddr, payLoad, fromAddress)
}
...
func (w *Wallet) txToOutputsInternal(outputs []*wire.TxOut, account uint32, minconf int32,
	...
	changeAddrStr string, payLoad []byte, fromAddress string) (*txauthor.AuthoredTx, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		atx, err = txauthor.NewUnsignedTransaction(outputs, txFee,
			inputSource.SelectInputs, changeSource, accType, w.chainParams, getScript, payLoad, fromAddress)
		if err != nil {
			return err
		}
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := hcutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisigInternal(dbtx walletdb.ReadWriteTx, account uint32,
	...
	txToMultisigError :=
		func(err error) (*CreatedTx, hcutil.Address, []byte, error) {
	...
	scAddr, err := hcutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(err)
	}
	...
	// Request updates from hcd for new transactions sent to this
	// script hash address.
	utilAddrs := make([]hcutil.Address, 1)
	utilAddrs[0] = scAddr
	err = chainClient.LoadTxFilter(false, []hcutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(err)
	}
...
func (w *Wallet) compressWallet(maxNumIns int, account uint32, changeAddr hcutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr hcutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) purchaseTickets(req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * hcutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy hcutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       hcutil.Amount(eopPool.amt),
...
func (w *Wallet) txToSStx(pair map[string]hcutil.Amount,
	inputCredits []udb.Credit, inputs []hcjson.SStxInput,
	payouts []hcjson.SStxCommitOut, account uint32, minconf int32) (*CreatedTx, error) {
...
func (w *Wallet) txToSStxInternal(dbtx walletdb.ReadWriteTx, pair map[string]hcutil.Amount,
	inputCredits []udb.Credit, inputs []hcjson.SStxInput,
	payouts []hcjson.SStxCommitOut) (tx *CreatedTx, err error) {
	...
	for _, input := range inputs {
		...
		if err != nil {
			return nil, hcjson.ErrDecodeHexString
		}

		if !(input.Tree == wire.TxTreeRegular ||
			input.Tree == wire.TxTreeStake) {
			return nil, hcjson.Error{
				Code:    hcjson.ErrInvalidParameter.Code,
		...
		totalAdded += hcutil.Amount(input.Amt)
	}
	...
	for i := range inputs {
		...
		if payouts[i].Addr == "" {
			...
			addr, err = hcutil.DecodeAddress(payouts[i].Addr)
			if err != nil {
				return nil, fmt.Errorf("cannot decode address: %s", err)
			}
			...
			default:
				return nil, hcjson.ErrInvalidAddressOrKey
			}
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			hcutil.Amount(payouts[i].CommitAmt), limits)
		if err != nil {
			return nil, fmt.Errorf("cannot create txout script: %s", err)
		}
		txout := wire.NewTxOut(int64(0), pkScript)
		...
		if payouts[i].ChangeAddr == "" {
			...
			a, err := hcutil.DecodeAddress(payouts[i].ChangeAddr)
			if err != nil {
				return nil, err
			}
			// Ensure the address is one of the supported types and that
		...
		err = addSStxChange(msgtx,
			hcutil.Amount(payouts[i].ChangeAmt),
			changeAddr)
		if err != nil {
			return nil, err
		}
...
func addSStxChange(msgtx *wire.MsgTx, change hcutil.Amount,
	changeAddr hcutil.Address) error {
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount hcutil.Amount, currentHeight int32) ([]udb.Credit, error) {
...
func signMsgTx(msgtx *wire.MsgTx, prevOutputs []udb.Credit,
	...
	for i, output := range prevOutputs {
		...
		apkh, ok := addrs[0].(*hcutil.AddressPubKeyHash)
		if !ok {
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr hcutil.Address) (chainec.PrivateKey, bool, error) {
	...
	var getScript txscript.ScriptClosure = func(addr hcutil.Address) ([]byte, error) {
		script, done, err := w.Manager.RedeemScript(addrmgrNs, addr)
		if err != nil {
			return nil, err
		}
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB hcutil.Amount) (*wire.MsgTx, error) {
	...
	for _, output := range revocation.TxOut {
		if hcutil.Amount(output.Value) > feeEstimate {
			amount := hcutil.Amount(output.Value) - feeEstimate
