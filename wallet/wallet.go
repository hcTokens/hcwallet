...
func (w *Wallet) StakeDifficulty() (hcutil.Amount, error) {
	...
	sd, err := hcutil.NewAmount(sdResp.NextStakeDifficulty)
	if err != nil {
		return 0, err
	}
...
// the wallet.
func (w *Wallet) requireChainClient() (*hcrpcclient.Client, error) {
	w.chainClientLock.Lock()
...
// of addresses loaded.
func (w *Wallet) loadActiveAddrs(dbtx walletdb.ReadTx, chainClient *hcrpcclient.Client) (uint64, error) {
	pool := sync.Pool{New: func() interface{} { return make([]hcutil.Address, 0, 256) }}
	recycleAddrs := func(addrs []hcutil.Address) { pool.Put(addrs[:0]) }
	getAddrs := func() []hcutil.Address { return pool.Get().([]hcutil.Address) }
...
func (w *Wallet) fetchHeaders(chainClient *hcrpcclient.Client) (int, error) {
	fetchedHeaders := 0
...
func (w *Wallet) FetchHeaders(chainClient *hcrpcclient.Client) (count int, rescanFrom chainhash.Hash, rescanFromHeight int32,
	...
	//
	// See https://github.com/HcashOrg/hcd/issues for details.  This hack
...
// finished.
func (w *Wallet) syncWithChain(chainClient *hcrpcclient.Client) error {
...
// spending to it in the blockchain or hcd mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (hcutil.Address, error) {
	...
	if buf.branchXpub.GetAlgType() == udb.AcctypeBliss {
		var blissaddr *hcutil.AddressPubKeyHash
		childIndex := buf.lastUsed + 1 + buf.cursor
		err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
			addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
			var err error
		...
		for _, branchKey := range []*hdkeychain.ExtendedKey{extKey, intKey} {
			...
			go func() {
				...
				if branchKey.GetAlgType() == udb.AcctypeBliss {
					addrs := make([]hcutil.Address, DefaultGapLimit)
					err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
						addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*hcutil.Amount)
...
func (w *Wallet) DumpWIFPrivateKey(addr hcutil.Address) (string, error) {
	...
	wif, err := hcutil.NewWIF(privKey, w.chainParams, privKey.GetType())
	if err != nil {
		return "", err
	}
...
func (w *Wallet) resendUnminedTxs(chainClient *hcrpcclient.Client) {
	var txs []*wire.MsgTx
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		var err error
...
// returning the total amount of hcd received for all accounts.
...
// returning the total amount of hcd received for a single wallet
// address.
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr hcutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr hcutil.Address) ([]byte, error) {
...
func Open(db walletdb.DB, pubPass []byte, privPass []byte, votingEnabled bool, addressReuse bool,
	...
	ticketFeeAmt, err := hcutil.NewAmount(ticketFee)
	if err != nil {
		return nil, err
	}

	relayFeeAmt, err := hcutil.NewAmount(relayFee)
	if err != nil {
		return nil, err
	}
